...
Lift_info *liftInfo;
semaphore drawMutex,floorMutex;
//
...
void getintolift(Lift_info *l,int direction) {
  ...
  while(*waiting) {    /* for all people waiting */
	 ...
	 if(l->peopleinlift<MAXNOINLIFT && *waiting) {
		...
		liftInfo = l;/* tell the person which lift it is */
		signal(*s);/* and wake them up */
	  }
	  else {
		 break;
	  }
...
unsigned long CALLBACK lift_thread(void *p) {
  ...
  while(1) {        /* do forever */
	 ...
	 while (l.stops[l.position]!=0) {  /* drop off all passengers */
		...
		signal(l.stopsem[l.position]);  /* tell them to get out */
...
unsigned long CALLBACK person_thread(void *p) {
  ...
  while(1) {     /* stay in the building forever */
	...
	l= liftInfo;  /* which lift are we geting in to */
	...
	wait(l->stopsem[to]);	/* wait until we get to the right floor */
